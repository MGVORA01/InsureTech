...
const PHASE_META: Record<Phase, { label: string; color: string }> = {
  ...
  restoring: { label: 'Restoring your session', color: 'var(--color-risk-low)' },
}
...
export default function MaintenancePage() {
  ...
  const checkingProgress = ((AUTO_RETRY_SECONDS - countdown) / AUTO_RETRY_SECONDS) * 100
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4 py-10">
      ...
      <div className="relative flex w-full max-w-lg flex-col items-center">
        <div className="mb-7 flex items-center gap-2.5">
          ...
          <span className="text-[17px] font-bold tracking-tight text-text-primary">InsureTech</span>
        </div>
        ...
        <div className="w-full rounded-[var(--radius-xl)] border border-border bg-surface p-8 text-center shadow-lg sm:p-10">
          <div className="relative mx-auto flex h-[76px] w-[76px] items-center justify-center">
            <span
              ...
              style={{ backgroundColor: 'var(--color-selected)', animation: 'maint-ping 2.2s cubic-bezier(0,0,0.2,1) infinite' }}
            />
            <span
              className="relative flex h-16 w-16 items-center justify-center rounded-full"
              style={{ backgroundColor: 'var(--color-selected)' }}
            >
              <BuildRoundedIcon style={{ color: meta.color, fontSize: 28, transition: 'color 300ms' }} />
            </span>
          </div>
          ...
          <p className="mx-auto mt-3 max-w-sm text-[14.5px] leading-6 text-text-secondary">
            InsureTech is currently undergoing scheduled maintenance to improve your experience.
            This won&rsquo;t take long &mdash; thanks for your patience. This page will refresh
            itself automatically the moment we&rsquo;re back.
          </p>

          <div className="mx-auto mt-8 w-full max-w-sm text-left">
            <div className="mb-2 flex items-center justify-between">
              <span className="inline-flex items-center gap-1.5 text-[12.5px] font-bold" style={{ color: meta.color }}>
                <span
                  className="h-1.5 w-1.5 rounded-full"
                  style={{ backgroundColor: meta.color, ...(phase !== 'checking' ? { animation: 'maint-pulse 1s ease-in-out infinite' } : {}) }}
                />
                {meta.label}
              </span>
              {phase === 'checking' && (
                <span className="text-[11px] font-semibold text-text-tertiary">
                  {AUTO_RETRY_SECONDS - countdown}s elapsed &middot; {Math.round(checkingProgress)}%
                </span>
              )}
              {phase === 'reconnecting' && (
                <span className="text-[11px] font-semibold text-text-tertiary">In progress&hellip;</span>
              )}
              {phase === 'restoring' && (
            ...
            <div className="h-2.5 w-full overflow-hidden rounded-full bg-[var(--color-border)]">
              {phase === 'checking' && (
                <div
                  ...
                  style={{ width: `${checkingProgress}%`, backgroundColor: meta.color }}
                />
              )}
              {phase === 'reconnecting' && (
                <div className="relative h-full w-full">
                  <div
                    ...
                    style={{ backgroundColor: meta.color, animation: 'maint-sweep 1.1s ease-in-out infinite' }}
                  />
                </div>
              )}
              {phase === 'restoring' && (
            ...
            <div className="mt-2 flex justify-between text-[10px] font-semibold uppercase tracking-[0.04em] transition-colors">
              <span style={{ color: phase === 'checking' ? PHASE_META.checking.color : 'var(--color-text-tertiary)' }}>
                Checking
              </span>
              <span style={{ color: phase === 'reconnecting' ? PHASE_META.reconnecting.color : 'var(--color-text-tertiary)' }}>
                Reconnecting
              </span>
              <span style={{ color: phase === 'restoring' ? PHASE_META.restoring.color : 'var(--color-text-tertiary)' }}>
                Restoring
              </span>
            </div>
          </div>
          ...
          >
            <RefreshRoundedIcon className={`h-[18px] w-[18px] ${busy ? 'animate-spin' : ''}`} />
            {phase === 'reconnecting' ? 'Checking…' : phase === 'restoring' ? 'Almost there…' : 'Check now'}
          </button>

          {attempts > 2 && phase === 'checking' && (
            <p className="mt-5 border-t border-border pt-4 text-[12px] leading-5 text-text-tertiary">
              Maintenance taking longer than expected? Please check back again shortly.
            </p>
          )}
        </div>
