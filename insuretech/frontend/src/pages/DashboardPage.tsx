    ...
    <div className="grid gap-7">
      <div className="grid gap-5 md:grid-cols-3">
        {Array.from({ length: 3 }).map((_, i) => (
          <div
            key={i}
            className="h-40 animate-pulse rounded-[18px] border border-black/5 bg-white/80 shadow-[0_14px_40px_rgba(20,20,19,0.04)]"
          />
        ))}
      </div>
...
function getAssessmentDate(results: ProfilingCompleteOut | null) {
  return (
    ...
    results?.session.updated_at ??
    results?.session.created_at ??
    null
  );
}
...
function getPolicyCount(results: ProfilingCompleteOut | null) {
  const policies = (
    results as (ProfilingCompleteOut & { policies?: unknown[] }) | null
  )?.policies;
...
function OverviewCards({
  ...
  topRecommendation: RecommendationOut | null;
  ...
  const recommendedPolicyName =
    topRecommendation?.policy_name ||
    topRecommendation?.policies?.[0]?.policy_name;
  const recommendedCompanyName =
    topRecommendation?.company_name ||
    topRecommendation?.policies?.[0]?.insurer_name;
  const hasRecommendation = Boolean(
    recommendedPolicyName || recommendedCompanyName,
  );
  ...
  const cards = [
    ...
    },
    {
      ...
      value: hasRecommendation
        ? (recommendedPolicyName ?? "Policy suggestion")
        : policyCount === null
          ? "--"
          : String(policyCount),
      supportText: hasRecommendation
        ? (recommendedCompanyName ?? "Top recommended policy")
        : results
          ? "Policy matching is ready after recommendations load"
          : "Recommendations unlock after profiling",
      badge: hasRecommendation
        ? policyCount
    ...
    },
    {
      ...
      supportText: results
        ? "Completed risk profiling session"
      ...
      {cards.map((card) => (
        ...
        >
          ...
          <div className="mt-4">
            ...
            <h3 className="mt-1 line-clamp-2 text-[28px] font-bold leading-[1.08] tracking-tight text-gray-950">
              {card.value}
            </h3>
  ...
  return (
    <section className="rounded-[18px] border border-black/5 bg-white shadow-[0_14px_40px_rgba(20,20,19,0.045)]">
      <div className="flex items-center justify-between gap-4 border-b border-black/5 px-5 py-5">
        <div>
          <h2 className="text-[20px] font-bold tracking-tight text-gray-950">
            Recent Activity
          </h2>
        ...
        {activities.map((activity) => (
          <div key={activity.id} className="flex items-start gap-3.5 py-5">
            <span
              ...
              style={{
                backgroundColor: `${activity.accent}12`,
            ...
            <div className="min-w-0">
              <h3 className="truncate text-[16px] font-bold leading-5 text-gray-950">
                {activity.title}
              </h3>
              <p className="mt-1 truncate text-[12px] font-medium leading-4 text-gray-400">
                {activity.timestamp}
              </p>
            </div>
          </div>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const resolveView = async () => {
      try {
        const results =
          await profilingApi.getBusinessResults(selectedBusinessId);
        if (cancelled) return;
        if (results) {
      ...
      return (
        <div
          ...
          style={{
            ...
            border: "1px solid var(--color-risk-medium-bg)",
    ...
    return (
      ...
      >
        <div className="mb-6">
          ...
          >
            {PROFILE_MESSAGES.noProfile}
          </p>
          <p
            className="mt-1 text-sm"
            style={{ color: "var(--color-text-tertiary)" }}
          >
            {PROFILE_MESSAGES.noProfileProfiling}
          </p>
        </div>
